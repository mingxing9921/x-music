...
export const home = (obj) => {
				return {type: HOME, obj}
}

export const recommendMusic = (obj) => {
				return {type: RECOMMENDMUSIC, obj}
}

export const scrollTop = (obj) => {
				return {type: SCROLLTOP, obj}
}
...
export function homeAPI(d, page) {
				...
				return async dispatch => {
								...
								try {
												...
												let musicList = await api(Config.musicListAPI, 'get', {
																page: page,
																json: true
												})

												data.banner = banner.data.info
												data.recommendMusics = (page === 1)
																? musicList.plist.list.info
																: d
																				.recommendMusics
